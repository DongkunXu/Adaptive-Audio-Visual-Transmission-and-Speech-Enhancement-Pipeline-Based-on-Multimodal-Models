...
class Step1SN(WizardStep):
    ...
    def __init__(self, parent, on_start_callback):
        ...
        self.btn = tk.Button (self, text ="Start >>", command=self._on_connect_selected)
...
class Step2Connected(WizardStep):
    ...
    def Run(self):
        self.scan_thread = threading.Thread(target=self._start_test1_open_open)
    ...
    def _start_test1_open_open(self):
        try:
            self.ntwk = self.driver.scan_skrf_network(helper.StandardFrequencyVector,5, self._on_progress)
            self.after(100, self._on_complete)
        except pocketvna.PocketVnaHandlerInvalid:
            self.after(100, self._on_failed)
            self.driver.close()
    ...
    def _on_progress(self, u, i):
        self._set_progress(  i * 100 / len(helper.StandardFrequencyVector) )
...
class Step3OpenOpenCheck(WizardStep):
    ...
    def Run(self):
        self.result = dict({})
        self._check_is_open_open(self.ntwk)

    def _check_is_open_open(self, ntwk):
        if ntwk is not None:
            dists, names = helper.find_nearest_network(ntwk)
            passed = True
            for p1 in range(0,2):
                for p2 in range(0,2):
                    if not self._bg_set_for_port(dists, names, p1, p2):
                        passed = False
    ...
    def _bg_set_for_port(self, dists, names, p1, p2):
        d    = dists[p1][p2]
        fn   = names[p1][p2]
        ...
        if self.is_requried_standard(p1, p2, d, standard):
            ...
            self._bg_show_plot(p1, p2, d, standard, helper.StandardNetworks[fn])
            cb.deselect()
            self.result['s{}{}-passed'.format(p1+1, p2+1)] = False
            return False

    def _bg_show_plot(self, p1, p2, d, standard, standardNtwk):
        if p1 == p2:
            self._bg_show_network_parameter(standardNtwk, self.ntwk, p1, p2, standard, d, 'reflective')
        if p2 != p1:
            self._bg_show_network_parameter(standardNtwk, self.ntwk, p1, p2, standard, d, 'transmissive')

    def _bg_show_network_parameter(self, standardNetwork, scannedNetwork, p1, p2, fittingStandard, dist, addition):
...
class MyApp(tk.Frame):
    ...
    def __init__(self, parent):
        ...
        self._set_title("Device Checker")
    ...
    def _on_serial_number_entered(self, sn):
        self.active_SerialNumber = sn        
        self._set_title("Device Checker: " + sn)
        self._connect_to_device()

    def _on_open_open_network_scanned(self, ntwk):
        self.open_open_network = ntwk
        self._jump_step()

    def _jump_to_starter(self):
        self._set_title("Device Checker")
        self.active_SerialNumber = None
        self.page_index = -1
        self.driver.close()
    ...
    def _on_short_load_network_scanned(self, ntwk):
        self.short_load_network = ntwk
        self._jump_step()

    def _on_test_complete(self, dictShortLoadCheckResult):
        self.short_load_check_result = dictShortLoadCheckResult
        self._save_results()
        self._jump_to_starter()
    ...
    def _connect_to_device(self):
        ...
        if self._set_appropriate_connection(driver):
            self.driver = driver
            ...
            driver.close()
            self.after(5000, self._connect_to_device)

    def _on_connection_lost(self):
        tk.messagebox.showerror("Connection Lost", "Device/Connection has disappeared :(")
        self._jump_to_starter()

    def _jump_2_previous(self):
        self.page_index -= 2
        self._jump_step()

    def _next_select_step(self):
        self.page_index += 1
        ...
        if self.page_index == 0:
            self.current_wizard_page = Step1SN(self, self._on_serial_number_entered)
        elif self.page_index == 1:
            self.current_wizard_page = Step2Connected(self, self.driver, self._on_open_open_network_scanned, self._on_connection_lost)
        elif self.page_index == 2:
            self.current_wizard_page = Step3OpenOpenCheck(self, self.open_open_network, self.on_next_SL_test, self._jump_2_previous)
        elif self.page_index == 3:
            self.current_wizard_page = Step4Connected(self, self.driver, self._on_short_load_network_scanned, self._on_connection_lost)
        elif self.page_index == 4:
            self.current_wizard_page = Step5ShortLoadCheck(self, self.short_load_network, self._on_test_complete, self._jump_2_previous)
        else:
            self.page_index, self.page_code = -1, 'nopage'
            self._next_select_step()
    ...
    def Run(self):
        self.pack()
        self.after(50, self._jump_step)
        self.mainloop()

    def _set_appropriate_connection(self, driver):
        if driver.count() < 1:
            return False
