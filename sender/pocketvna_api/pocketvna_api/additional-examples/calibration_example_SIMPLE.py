...
# ------------- Compensation algorithm application --------------------------
##   *** Fill all data with Zeros. If some Parameter is not supported by the calibration file then it's data will be Zeros
calibratedS = numpy.zeros((len(dut.f), 2, 2), dtype=numpy.complex128)

if interpolatedCalibrationData.isS11Ready():
    calibratedS[:, 0, 0] = reflection_compensation(interpolatedCalibrationData.referenceResistance(),\
        dut.s[:, 0, 0],\
        interpolatedCalibrationData.shortS11(), interpolatedCalibrationData.openS11(), interpolatedCalibrationData.loadS11())

if interpolatedCalibrationData.isS21Ready():
    calibratedS[:, 1, 0] = transmission_compensation( dut.s[:, 1, 0],  \
                    interpolatedCalibrationData.openS21(), interpolatedCalibrationData.thruS21() )

if interpolatedCalibrationData.isS12Ready():
    calibratedS[:, 0, 1] = transmission_compensation( dut.s[:, 0, 1], \
        interpolatedCalibrationData.openS12(), interpolatedCalibrationData.thruS12() )

if interpolatedCalibrationData.isS22Ready():
    calibratedS[:, 1, 1] = reflection_compensation(interpolatedCalibrationData.referenceResistance(),\
        dut.s[:, 1, 1],\
        interpolatedCalibrationData.shortS22(), interpolatedCalibrationData.openS22(), interpolatedCalibrationData.loadS22())

##   *** Make skrf Network object for convenience
