...
# -----------  S11  ---------------
    ...
    def isS11Ready(self):
        return not self.isEmpty() and \
            len(self.frequency()) == len(self.shortS11()) and \
            len(self.frequency()) == len(self.openS11()) and \
            len(self.frequency()) == len(self.loadS11())
...
# -----------  S21  ---------------
    ...
    def isS21Ready(self):
        return not self.isEmpty() and \
                len(self.frequency()) == len(self.openS21()) and \
                len(self.frequency()) == len(self.thruS21())
...
# -----------  S12 ---------------
    ...
    def isS12Ready(self):
        return not self.isEmpty() and \
                len(self.frequency()) == len(self.openS12()) and \
                len(self.frequency()) == len(self.thruS12())
...
# ------- S22 ------------------
    ...
    def isS22Ready(self):
        return not self.isEmpty() and \
            len(self.frequency()) == len(self.shortS22()) and \
            len(self.frequency()) == len(self.openS22()) and \
            len(self.frequency()) == len(self.loadS22())
        ...
        for title in columntitles:
            ...
            if (array is not None):
                if not is_empty(array):
                    new_data.dataMap()[title]= interpolate_array(self.frequency(), numpy.array(array, dtype=numpy.complex128), newf)
...
class TOSMCalibrationData(CalibrationData):
    ...
    def isReady(self):
        return not self.isEmpty() and \
                len(self.shortS11()) == len(self.frequency()) and len(self.openS11()) == len(self.frequency()) and len(self.matchS11()) ==  len(self.frequency()) and \
                len(self.thruS11())  == len(self.frequency()) and len(self.thruS21()) == len(self.frequency()) and  \
                len(self.shortS22()) == len(self.frequency()) and len(self.openS22()) == len(self.frequency()) and len(self.matchS22()) ==  len(self.frequency()) and \
                len(self.thruS22())  == len(self.frequency()) and len(self.thruS12()) == len(self.frequency()) and \
                (is_empty(self.matchS21()) or len(self.matchS21()) == len(self.frequency())) and \
                (is_empty(self.matchS12()) or len(self.matchS12()) == len(self.frequency()))
...
# - -
    ...
    def gen_match_match_network(self):
        ...
        size = len(freq)
        z0   = self.referenceResistance()
        return build_network(freq, ensure_length(size,self.matchS11()),  ensure_length(size,self.matchS21()),\
                                    ensure_length(size,self.matchS12()),  ensure_length(size,self.matchS11()), z0)

    def gen_thru_network(self):
        freq = self.frequency()
        size = len(freq)
        z0   = self.referenceResistance()
        return build_network(freq, self.thruS11(),  self.thruS21(),\
                                   self.thruS12(),  self.thruS22(), z0)
...
class CalibrationFileLoader:
    ...
    CALIBRATOIN_DATA_START_TAG = "{startcali:" # Most important section. Everything else may be count as auxiliary info
    CALIBRATION_DATA_END_TAG   = "}endcali"
    COMMENT_TAG                = '#'

    CALKIT_MODEL_START_TAG      = "{calkit_start:"
    CALKIT_MODEL_END_TAG        = "}calkit_end"
    CALKIT_MEASUREMENT_START_TAG= "{calkit_measurements_start:"
    CALKIT_MEASUREMENT_END_TAG  = "}calkit_measurements_end"
    ...
    CALKIT_NAME_TAG  =  CALIBRATION_OPTION_TAG + "calkit"+ ":"
    LOADSTD_TAG      =  CALIBRATION_OPTION_TAG + "loadstd"+":"
    FREQ_COLUMN      =  "frq"
    ...
    def _read_cali_type(self):
        headLine = self.__read_line()
        if headLine is None:
            return None

        if headLine.startswith(CalibrationFileLoader.CALIBRATION_FILE_TAG):
            if   headLine.startswith(CalibrationFileLoader.CALIBRATION_FILE_TAG + "#" + CalibrationFileLoader.CALIBRATION_LMR16):
                return CalibrationAlgo.LMR16
            elif headLine.startswith(CalibrationFileLoader.CALIBRATION_FILE_TAG + "#" + CalibrationFileLoader.CALIBRATION_SIMPLE):
                return CalibrationAlgo.SIMPLE_SOLT
            elif headLine.startswith(CalibrationFileLoader.CALIBRATION_FILE_TAG + "#" + CalibrationFileLoader.CALIBRATION_TOSM):
                return CalibrationAlgo.TOSM
            elif headLine.startswith(CalibrationFileLoader.CALIBRATION_FILE_TAG + "#" + CalibrationFileLoader.CALIBRATION_TRL):
    ...
    def _read_settings(self):
        ...
        while True:
            ...
            elif line.startswith(CalibrationFileLoader.CALKIT_MODEL_START_TAG):
                print('Contains Calibration Kit model definition. Ignoring')
                # Skip model lines. Model usually has Short,Open,Load,Through definitions.
                # Parameters are polynomial: Offset delay,Offset Z0, loss, C[0]-C[3] F/Hz^[n] or L[0]-L[3] H/Hz^[n]
                # skip them all
                while line != CalibrationFileLoader.CALKIT_MODEL_END_TAG:
                    line = self.__read_line()
            elif line.startswith(CalibrationFileLoader.CALKIT_MEASUREMENT_START_TAG):
                print('Contains Calibration Kit measurements. Ignoring')
                # Skip calkit measurements' lines. Usually contains Parameters Network for Short,Open,Load [Through]
                # Each network's line: frequency;S-real;S-imag
                # skip them all
                while line != CalibrationFileLoader.CALKIT_MEASUREMENT_END_TAG:
                    line = self.__read_line()
            elif line.startswith(CalibrationFileLoader.COMMENT_TAG):
                pass # just a comment, may contain useful info for a user but nothing for a soft
            elif not line:
                # line is empty
                pass
            else:
    ...
    def _parse_cell(self, columnindex, cellstr):
        ...
        if self.isFrequency(key):
            ...
            assert len(complexes) == 2, "Complex number should contain 2 elements line#" + str(self.current_line_number) + " at " + str(columnindex+1)
    ...
    def _process_option(self, line):
        ...
        elif line.startswith(CalibrationFileLoader.LOADSTD_TAG):
            # Kind of calkit for Simple Compensation
            self.loadstdz0 = float(line.replace(CalibrationFileLoader.LOADSTD_TAG, '').strip())
            print("Contains custom Match Standard!: ", self.loadstdz0)
        elif line.startswith(CalibrationFileLoader.CALKIT_NAME_TAG):
            # It uses some build-in predefined set of measurements for a given CalKit name. For example 'built-in/uflhirose'; 'built-in' for male-SMA; so on
            print("Calkit tag found. Ignoring: ", line.replace(CalibrationFileLoader.CALKIT_NAME_TAG, '').strip())
        else:
