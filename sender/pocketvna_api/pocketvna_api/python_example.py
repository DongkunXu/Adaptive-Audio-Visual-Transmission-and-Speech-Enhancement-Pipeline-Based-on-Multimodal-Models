...
pocketvna.set_driver_option(pocketvna.DriverOptionsEnum.LogLevel, pocketvna.DriverOptionsEnum.LogLevelsEnum.debug)
...
try:
    ...
    ## Get valid frequency range. Usually [1; 6G]Hz for PocketVNA v1.5-v2.2. It is a range which device can handle
    ...
    if firmware_version < 0x300:
        print('1 GHz FULL Scan (S11, S21, S12, S22 parameters): ')
        s11, s21, s12, s22 = driver.single_scan(1000000000, average,  pocketvna.NetworkParams.ALL_2P)
        ...
        print('single_scan is obsolete and belongs to previous devices PocketVNA v1.0 to v2.2')
    ...
    s11, s21, s12, s22 = driver.scan(freq, average,  pocketvna.NetworkParams.Util2Port.AllSupported)
    ...
    for i in range(0, num_of_points):
        print( 'P1 Expected: (' + str(math.pi / (i + 1)) + '; ' + str(1. / (math.pi / (i + 1))) + 'j)\t' )
        print( '   REAL: ('     + str(p1[i].real)        + '; ' + str(p1[i].imag) + 'i)\n' )

        print( 'P2 Expected: (' + str(math.pi * i)       + '; ' + str((math.pi ** (i + 1))) + 'j)\t' )
